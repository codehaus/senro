    ...
    throws IOException {
        ...
        if (reader != null) {
            ...
            int markDistance = ((Integer)doc.getProperty(
                                    SettingsNames.READ_MARK_DISTANCE)).intValue();
            ...
            boolean lastCR = false; // Last character in the previous buffer was '\r'
            ...
            int nextMarkOffset = markDistance; // Buffer offset of the next mark insertion
            int nextMarkPos = markDistance; // Position where the next mark should be inserted
...
// The property above is left empty so the write() will default to the READ_LINE_SEPARATOR_PROP
                    ...
                    for (offset = 0; offset < readLen; offset++) {
                        ...
                        case '\n':
                            if (lastCR) { // ignore this LF
                                ...
                                } else { // chars yet start from the begining of buffer
                                    tgtOffset = offset;
                                }
                                moveStart = offset + 1;
                                lastCR = false;
                ...
                // Mark the inserted area is not yet covered by syntax state-infos
