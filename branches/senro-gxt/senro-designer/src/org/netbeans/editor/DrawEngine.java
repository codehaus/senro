...
class DrawEngine {
    ...
    private void initInfo(DrawInfo ctx) throws BadLocationException {
        ...
        ctx.doc.op.prepareSyntax(ctx.slot, ctx.syntax,
            ctx.doc.op.getLeftSyntaxMark(ctx.startOffset),
            ctx.startOffset, ctx.endTokenMarkOffset - ctx.startOffset,
            false
        );
    ...
    */
    ...
    private void computeFragmentLength(DrawInfo ctx) {
        ...
        int stopIndex = Analyzer.findFirstTabOrLF(ctx.slotArray,
        ...
        if (stopIndex >= 0) { // either '\t' or '\n' found
            if (stopIndex == ctx.fragmentStartIndex) { // since fragment start
                if (ctx.slotArray[stopIndex] == '\t') { //
                    ...
                    int ntInd = Analyzer.findFirstNonTab(ctx.slotArray, ctx.fragmentStartIndex,
                                                         ctx.fragmentLength);
    ...
    private void computeFragmentDisplayWidth(DrawInfo ctx) {
        ...
        if (ctx.tabsFragment) { // tabs in fragment
            ctx.fragmentCharCount = Analyzer.getColumn(ctx.slotArray,
        ...
        // Check whether targeting all characters
          ...
          && ctx.fragmentLength > 0 // Not sure whether it's necessary
            ...
            if (ctx.tabsFragment) { // fragment contains tabs
                int spaceCount = Analyzer.getColumn(ctx.slotArray,
            ...
            ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(
                ctx.fragmentOffset + high, ctx.slotArray[ctx.fragmentStartIndex + high],
            ...
            if (!ctx.continueDraw) {
                ...
                while (low <= high) {
                    ...
                    ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(
                        ctx.fragmentOffset + mid, ctx.slotArray[ctx.fragmentStartIndex + mid],
        ...
        // Check whether target offset is inside current fragment
            ...
            if (i > 0) {
                if (ctx.tabsFragment) { // fragment contains tabs
                    ...
                    prevWidth = FontMetricsCache.getFontMetrics(ctx.font,
                        ctx.component).charsWidth(ctx.slotArray, ctx.fragmentStartIndex, i);
                }
            }
            ...
            if (ctx.tabsFragment) { // fragment contains tabs
                ...
                curWidth = FontMetricsCache.getFontMetrics(ctx.font,
                           ctx.component).charsWidth(ctx.slotArray, ctx.fragmentStartIndex, i + 1);
            }

            ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(
                ctx.fragmentOffset + i, ctx.slotArray[ctx.fragmentStartIndex + i],
    ...
    */
    ...
    class DrawInfo extends DocMarks.Renderer implements DrawContext {
        ...
        */
         ...
         * of the drawing and in which the prescan is low enough so that
         * it's clear that the last token will not reach this offset.
         * It's computed by mark renderer.
         */
        ...
        MarkFactory.DrawMark rangeDrawMarkArray[]
        = new MarkFactory.DrawMark[DEFAULT_DRAW_MARK_RENDERER_SIZE];

        /** Array of positions of the draw-marks that were found in the draw area. */
        ...
        public void render() {
            ...
            while (true) {
                if (mark instanceof MarkFactory.SyntaxMark) {
                    ...
                    if (si == null) {
                        /* This is a situation where there's a long token
                         * at the end of the area to be drawn. Normally the 
                         * prepareSyntax() should update some area around
                         * but if the token is long the stateinfo can
                         * be empty which is this situation. It has no sense
                         * to go on because all the following state infos
                         * are null. So in this case the the prepareSyntax()
                         * is made for the end of the document to be sure
                         * all the marks are updated. The slot must
                         * be (and it should be) empty at this time.
                         */
                        try {
                            doc.op.prepareSyntax(slot, syntax, 
                                     doc.op.getLeftSyntaxMark(startOffset),
