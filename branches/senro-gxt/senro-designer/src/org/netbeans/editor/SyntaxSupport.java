    ...
    */
    ...
    throws BadLocationException {
        ...
        try {
            ...
            doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startPos),
                                 startPos, 0, forceLastBuffer, forceNotLastBuffer);
            int preScan = syntax.getPreScan();
    ...
    */
    ...
    boolean forceLastBuffer) throws BadLocationException {
        ...
        try {
            ...
            int docLen = doc.getLength();
            doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startOffset),
                                 startOffset, endOffset - startOffset, forceLastBuffer);
            int preScan = syntax.getPreScan();
            tp.nextBuffer(slot.array, syntax.getOffset(), endOffset - startOffset,
            ...
            while (cont) {
                ...
                if (tokenID == null) { //EOT
                    ...
                    if (nextLen == 0) {
                        ...
                        syntax.relocate(slot.array, slot.offset + preScan, nextLen,
