...
public class SenroDateFieldBeanFactory extends JComponentBeanFactory
{
    private static final Set<String> BASIC_PROPERTIES =
            Collections.unmodifiableSet(new HashSet<String>(Arrays.asList("id")));

    public SenroDateFieldBeanFactory()
    {
        super(SenroDateField.class);
    }
    ...
    public JETABean createBean(String compName, boolean instantiateBean, boolean setDefaults) throws FormException
    {
        Component comp = null;
        if (instantiateBean) {
            comp = new SenroDateField();
            SenroDateField dateField = (SenroDateField) comp;
            dateField.setName(compName);
        }
        DynamicBeanInfo beaninfo = JComponentBeanFactory.createBeanInfo(SenroDateField.class);
