...
/**
 * Command that moves a component from one location and replaces
 * a component at another location.  Keep in mind that every cell
 * in the designer view has a component, so you would never have a
 * case where your are moving to an empty cell.  Rather you would move
 * and replace an EmptyComponent in that cell.
 * @author Jeff Tassin
 */
public class MoveComponentCommand extends FormUndoableEdit 
{
   ...
   public void redo() throws CannotRedoException
   {
	 ...
	 m_replace_cmd.redo();
	 /**
	  * We need to update the entire container hierarchy because it seems that
	  * nested forms can get confused when the delete and replace commands
	  * directly follow one another.  This can be tested by running the replace
	  * command in a SwingUtilities.invokeLater event.  It runs fine, but the screen
	  * has an annoying flicker. Walking up the container hierarchy and calling revalidate
