...
public class DefaultContainedFormFactory implements ContainedFormFactory
{
     ...
     * form forms that can be edited in-place in the designer.
     */
    public FormComponent createContainedForm(Class swingClass, FormMemento fm, GridType grid_type) throws FormException
    {
        ...
        switch(grid_type) {
            ...
            case GRID_VIEW:
            default:
                embedded_fac = new EmbeddedFormComponentFactory(compsrc);
                break;
        }
     ...
     * that we can edit. This is used in two cases. In the first, we use a top
     * level form in the FormEditor. In the second, we use the top-level form in
     * contained forms (e.g. a form that is contained in a JTabbedPane tab ).
     * <p/>
        ...
        throws FormException
    {
        ...
        /**
         * Now get the FormContainerComponent instance and add it as a listener to
         * the newly created form. This is needed so that GridComponent events
         * such as cell_selected and cell_changed are properly propagated up the
         * form hierarchy.
         */
