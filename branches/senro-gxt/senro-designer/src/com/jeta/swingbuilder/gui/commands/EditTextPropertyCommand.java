...
/**
 * Command that edits the text property of a component if that component has
 * a text property.
 * @author Jeff Tassin
 */
   ...
   public static void tryEditTextProperty( FormEditor editor, FormComponent fc, GridComponent gc )
   {
	 ...
	 try
	 {
	    ...
	    if ( comp != null )
	    {
		  ...
		  /** override the ENTER key handling for the dialog.  We need to do this because the ENTER key
		   * is bound to setting the Text property.  However, this key also closes a dialog.  The dialog can sometimes
		   * get the ENTER event for editing the text and close prematurely */
		  ...
		  newvalue = javax.swing.JOptionPane.showInputDialog( editor, I18N.getLocalizedMessage( "Set Text Property" ), oldvalue );
	       ...
	       if ( newvalue != null )
	       {
		  // if we are here, then the component has both set and get methods, so
