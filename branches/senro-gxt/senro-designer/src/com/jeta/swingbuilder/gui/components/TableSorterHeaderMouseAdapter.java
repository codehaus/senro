...
/**
 * This class listens for mouse events on the column headers of a 
 * sorted table. When the user clicks a column, this class invokes
 * the appropriate sort routine for that column.
 *
 * @author Jeff Tassin
 */
class TableSorterHeaderMouseAdapter extends MouseAdapter
{
   ...
   /**
    * @return the underlying table sorter. This will be null if the table has been gc'ed
    */
   ...
   /**
    * @return the underlying table. This will be null if the table has been gc'ed
    */
   JTable getTable()
   {
   ...
   public void mouseClicked(MouseEvent e) 
   {
      ...
      // sometimes the controller can be null here even if not garbage collected
	 ...
	 if (e.getClickCount() == 1 && modelcolumn != -1) 
	 {
	    ...
	    if ( shiftpressed !=0 && ctrlpressed != 0 )
	       ...
	       mode = SortMode.DESCENDING; // user explicitly chose descending sort
	    else if ( ctrlpressed != 0 )
	       mode = SortMode.ASCENDING;  // user explicitly chose ascending sort
	    else
	    {
