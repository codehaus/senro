...
import com.jeta.swingbuilder.gui.main.CellConstraintsController;
...
/**
 * Mouse event handler for a standard grid cell.  This is for
 * a cell that contains any component except a child form (i.e. GridView)
 *
 * @author Jeff Tassin
 */
public class StandardCellHandler extends AbstractMouseHandler
{
    ...
    /**
     * @return true if the cell associated with this handler contains the mouse point for the given event.
     */
    private boolean containsMouse(MouseEvent e)
    {
        ...
        if(local_pt.x >= m_cell.getCellX() && local_pt.x < (m_cell.getCellX() + m_cell.getCellWidth()) &&
           local_pt.y >= m_cell.getCellY() && local_pt.y < (m_cell.getCellY() + m_cell.getCellHeight())) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * @return the component under the given mouse. This can be different that the component
     *         associated with this handler if another component spans multiple cells and overlaps the
     *         cell occupied by this component.
     */
    ...
    /**
     * MouseMotionListener implementation. Note that the source will always be the top level window. So, we need to convert
     * the mouse point to the coordinates of our associated overlay window before any other operation.
     */
    public void mouseMoved(MouseEvent e)
    {
        if(!m_compsrc.isSelectionTool() || isDragging()) {
            ...
            if(gc != null) {
                /**
                 * If the component is not the component associated with this handler, let's just forward the call
                 * to that component. The reason is the component might span multiple cells and it might be a child form.
                 */
                if(gc != m_cell) {
    ...
    public void mousePressed(MouseEvent e)
    {
        if(containsMouse(e)) {
            ...
            if(gc != null && editor != null) {
                /**
                 * If the component is not the component associated with this handler, let's just forward the call
                 * to that component. The reason is the component might span multiple cells or it might be a child form.
                 */
                if(gc != m_cell) {
                    ...
                    if(m_compsrc.isSelectionTool()) {
                        ...
                        try {
                            ...
                            if(factory != null) {
                                ...
                                if(comp != null) {
                                    /**
                                     * Special case when adding a linked form we need to check if the target
                                     * editor does not already contain that linked form
                                     */
                                    if(comp instanceof FormComponent) {
                                        FormComponent fc = (FormComponent)comp;
                                        if(fc.isLinked()) {
                                            if(FormManagerDesignUtils
                                                .containsForm(editor.getFormComponent(), fc.getId())) {
                                                String msg = I18N.format(
                                                    "Only one instance of a linked form allowed per view.", fc.getId());
                                                String title = I18N.getLocalizedMessage("Error");
                                                JOptionPane
                                                    .showMessageDialog(editor, msg, title, JOptionPane.ERROR_MESSAGE);
                                    ...
                                    AddComponentCommand cmd =
                                        new AddComponentCommand(getView().getParentForm(), comp, gc.getConstraints());
            ...
            if(gc != null && editor != null) {
                ...
                if(gc != m_cell) {
                    ...
                    if(getDragSource() != this) {
                        ...
                        MoveComponentCommand cmd =
                            new MoveComponentCommand(destForm, destComp, srcForm, srcComp, m_compsrc);
                        CommandUtils.invoke(cmd, editor);
                        m_compsrc.setSelectionTool();
                        srcComp.setSelected(true);
                        /** we need to do this in case the user tried to drag a top-level form from inside a JTabbedPane
    ...
    private void editComponentName(GridComponent gc)
    {
        ...
        if(comp != null) {
            ...
            String newvalue =
                javax.swing.JOptionPane.showInputDialog(comp, I18N.getLocalizedMessage("Set Name"), oldname);
            if(newvalue != null) {
                comp.setName(newvalue);
                gc.fireGridCellEvent(
                    new GridCellEvent(GridCellEvent.EDIT_COMPONENT, gc, GridCellEvent.COMPONENT_NAME_CHANGED));
            }
        }
    ...
    /**
     * Checks if the component has a getText and setText property.  If so, lauches a JOptionPane
     * to allow the user to change the property in place.
     */
