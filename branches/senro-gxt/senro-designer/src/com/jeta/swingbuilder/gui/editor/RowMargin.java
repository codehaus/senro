...
public class RowMargin extends Margin
{
   ...
   /**
    * ctor
    * @param view the top level view that will render the resize indicator
   ...
   void update( GridComponent gc )
   {
	 ...
	 if ( gc != null )
	 {
	    ...
	    else
	    {
		  ...
		  Point pt = javax.swing.SwingUtilities.convertPoint( parentview, 0, parentview.getRowOrgY( row_offset ) + parentview.getRowHeight( row_offset ) + insets.top, this );
		  m_ypos = pt.y;
	       }
	    }
	 }
	 ...
	 if ( m_dragging && m_gc != null )
	 {
	    if ( m_comp_size > 0 )
	    {
		  ...
		  else
		  {
		     ...
		     {
			msg = I18N.getLocalizedMessage( "The row size is set to bounded.\nYou must manually set the size in the row specification window." );
		     }
		     else if ( adapter.isResizeGrow() )
		     {
			msg = I18N.getLocalizedMessage( "The row resize behavior is set to grow.\nYou must manually set the size in the row specification window." );
		     }
		     else
		     {
