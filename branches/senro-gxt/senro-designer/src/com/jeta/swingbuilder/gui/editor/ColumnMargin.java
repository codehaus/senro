...
public class ColumnMargin extends Margin
{
   ...
   /**
    * ctor
    * @param formcomponent the main formcomponent that contains the view that will render the resize indicator
    * @param viewport the viewport that contains the top level view.  Needed when the view
    * is scrolled right or down
    */
   ...
   void update( GridComponent gc )
   {
	 ...
	 if ( gc != null )
	 {
	    ...
	    else
	    {
		  ...
		  Point pt = javax.swing.SwingUtilities.convertPoint( parentview, parentview.getColumnOrgX( col_offset ) + parentview.getColumnWidth( col_offset ) + insets.left, 0, this );
		  m_xpos = pt.x;
	       }
	    }
	 }
	 ...
	 if ( m_dragging && m_gc != null )
	 {
	    if ( m_comp_size > 0 )
	    {
		  ...
		  else
		  {
		     ...
		     {
			msg = I18N.getLocalizedMessage( "The column size is set to bounded.\nYou must manually set the size in the column specification window." );
		     }
		     else if ( adapter.isResizeGrow() )
		     {
			msg = I18N.getLocalizedMessage( "The column resize behavior is set to grow.\nYou must manually set the size in the column specification window." );
		     }
		     else
		     {
