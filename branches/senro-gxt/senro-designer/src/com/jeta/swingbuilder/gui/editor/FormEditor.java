...
{
    /**
     * The margin at the top of the view for selecting/editing/resizing column settings
     */
    private Margin m_colmargin;

    /**
     * The margin at the left of the view for selecting/editing/resizing row settings
     */
    ...
    /**
     * The object that determines the currently selected component to
     * create when adding a component to the form.
     */
    ...
    /**
     * The row/column cells on the status bar. Displays the row/column specs for the selected component
     */
    ...
    /**
     * The row/column specs cells. Display the row and column specs for the selected form.
     */
    ...
    /**
     * For debugging only
     * Displays the form id for the current parent and form components.
     */
     ...
     * forms in this editor. We keep track of this list in case the form
     * changes such as during a Save As operation.  In this situation, we can re-register
    ...
    /**
     * Clears all undo/redo edits for this editor. This is typically performed after
     * the editor has been saved.
     */
    ...
    /**
     * Creates a debug bar for this editor.  Shows extra information such as form id.
     */
    ...
    private FormComponent createTopParent(FormComponent form) throws FormException
    {
        ...
        /** you must call setOpaque here or there will be some color artifacts visible when look and feel changes */
        m_topparent.setOpaque(true);

        /**
         * Re-add any listeners to the top parent. This is needed when doing a Save As on
         * a form. The editor does not change but the form does.
         */
    ...
    /**
     * @return the editor that is an ancestor of the given component.  If the comp is not
     *         in a FormEditor hierarchy, null is returned.
     */
    ...
    /**
     * @return the top most overlay on the form.  This is the 1x1 form that  is the
     *         parent for the main form we are editing.
     */
    ...
    /**
     * @return the top most parent FormComponent.  This is the 1x1 form that  is the
     *         parent for the main form we are editing.
     */
    ...
    {
        m_undomanager =
            new EditorUndoManager((AbstractFormManager)JETARegistry.lookup(AbstractFormManager.COMPONENT_ID), this);
        ...
        /** we need to delay adding the listeners because the form component might not be fully initialized yet */
        ...
        /**
         * This mouse listener is for clicks in the border around the main form. It gives the user a little
         * room to select the main form
         */
    ...
    /**
     * @return true if this form is a linked form.  A linked form is
     *         one that is actually stored in a file.
     */
    ...
    public void setFocusViewVisible(boolean bvisible)
    {
        ...
        /*
        JLayeredPane layeredpane = gridview.getLayeredPane();
        if ( bvisible )
        {
       if ( m_focus_view != null )
       {
          layeredpane.remove( m_focus_view );
       }
       m_focus_view = new FocusView( m_form );
       layeredpane.add( m_focus_view, GridView.FOCUS_LAYER );
        }
        else
        {
       if ( m_focus_view != null )
       {
          if ( layeredpane.isAncestorOf( m_focus_view ) )
          {
             layeredpane.remove( m_focus_view );
          }
          m_form.setFocusPolicy( m_focus_view.getFocusPolicyMemento() );
    ...
    /**
     * Sets the formcomponent that is this form.  This call is made when the user performs
     * a Save As.
     */
    ...
    public void unitTest()
    {
        if(FormDesignerUtils.isTest()) {
            //com.jeta.swingbuilder.test.JETATestFactory.runTest( "test.jeta.swingbuilder.gui.editor.EditorValidator", this );
        }
    }
    ...
    /**
     * Updates the status bar to display the column and row specs for the given component.
     */
    ...
    /**
     * LayoutManager for this view.  It lays out the Margins and the main scroll pane
     */
