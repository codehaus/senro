...
{
   /**
    * Safely returns the classname of the bean delegate associated with this grid component.
    * Null is returned if there is no delegate associated with the component.
    */
   ...
   public static void invoke( FormUndoableEdit edit, FormEditor editor )
   {
	 ...
	 return;
      ...
      /**
       * Special case for SetPropertyCommand because we can get repeated commands
       * for the same property value given the way the property editor works.
       */
   ...
   /**
    * Redoes and edit for the given form.  If the redo is successful, it is propagated to all other editors
   ...
   /**
    * Undoes and edit for the given form.  If the undo is successful, it is propagated to all other editors
    * that have the same form.
    */
   public static FormUndoableEdit undoEdit( FormEditor editor )
   {
	 ...
	 /**
	  * Now, post the edit to all other editors that contain the form so they can add their
	  * undo managers can mark this edit as undone.
	  */
	 EditorManager emgr = (EditorManager)JETARegistry.lookup( EditorManager.COMPONENT_ID );
	 Collection editors = emgr.getEditors();
	 Iterator iter = editors.iterator();
