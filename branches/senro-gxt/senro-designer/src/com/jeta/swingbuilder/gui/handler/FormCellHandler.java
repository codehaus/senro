...
public class FormCellHandler extends AbstractMouseHandler
{
    ...
    private GridComponent getComponent(MouseEvent e)
    {
        ...
        if(cell == null) {
            ...
            if(parent != null) {
                ...
                local_pt =
                    SwingUtilities.convertPoint((Component)e.getSource(), e.getPoint(), (Component)parentoverlay);

                int y = local_pt.y;
                int x = local_pt.x;

                if(x >= m_comp.getCellX() && x <= m_comp.getCellX() + m_comp.getCellWidth() &&
                   y >= m_comp.getCellY() && y <= m_comp.getCellY() + m_comp.getCellHeight()) {
                    cell = m_comp;
                }
            }
        }
        ...
        if(isDragging()) {
            ...
            if(cell == m_comp) {
                if(getDragSource() != this) {
                    // mouse released on this entire form, so we replace the form.

                }
            } else if(cell != null) {
                if(getDragSource() != this) {
                    // don't allow drops on children of this form if we are the drag source
                    CellMouseHandler handler = cell.getMouseHandler();
                    if(handler != null) {
    ...
    public void mouseMoved(MouseEvent e)
    {
        ...
        if(cell != null) {
            CellMouseHandler handler = cell.getMouseHandler();
            if(handler == null) {
                //System.out.println( "found null handler  at row: " + cell.getRow() + "  col: " + cell.getColumn() + "  for: " + cell.getComponent() );
                assert (false);
            } else {
                if(getDragSource() != this) {
                    if(handler != this) {
