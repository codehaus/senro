...
public class PanelWriter
{
   ...
   public MethodWriter createPanel( DeclarationManager decl_mgr, FormMemento fm)
   {
	 ...
	 CellConstraints cc = ccm.createCellConstraints();
         ...
         try
         {
		  ...
		  if ( beanclass.indexOf( "GridView" ) > 0 || beanclass.indexOf( "JETALabel" ) > 0 || decl_mgr.isIncludeNonStandard() || beanclass.indexOf( "com.jeta" ) < 0 )
		  {
   ...
   /**
    * Creates an array of column or row indices that correspond to cells
    * in the first row or column that require empty filler components. This
    * is required to keep the spacing of the grid consistent with the view in the editor.
    * @param indexes a map of Integer indexes to ComponentMememeto objects.
    */
   ...
   private static class FillMarker
   {
	 ...
	 else 
	 {
      ...
      {
	 return ( m_memento == null || 
		  m_memento.getBeanClass() == null && 
		  (spec.getSize() == Sizes.DEFAULT || 
		   spec.getSize() == Sizes.PREFERRED) );
