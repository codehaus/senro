...
class DocOp implements AbstractDocument.Content {
    ...
    */
    ...
    /** Size of one batch for updating syntax marks. It's used in two cases.
    * When the prepareSyntax() is called it may be necessary to update
    * the state infos of the syntax marks that were not updated yet.
    * In this case the SYNTAX_UPDATE_BATCH_SIZE is the minimum area
    * after the required position that will be updated (required length
    * is often 0 so this prevents updating marks one by one).
    * The other case is when insert or remove is performed it's
    * not clear how many marks will it be necessary to update so
    ...
    */
    ...
    /** Position of the end of the already scanned area. When the initial read
    * is performed, it's set to zero. The <tt>prepareSyntax()</tt> updates
    * this position if necessary.
    * The value -1 means that either the document is empty or all the syntax
    * marks are already up-to-date. It happens by requesting the scanning
    * at the end of the document or after the first modification.
    * The particular value means that all the syntax marks including those
    * right at the position given by the value of the variable are up-to-date
    * even if there would be more than one mark right at that position.
    */
    ...
    */
    ...
    void setDocument(BaseDocument doc) {
        ...
        MARK_DISTANCE = ((Integer)doc.getProperty(
                             SettingsNames.MARK_DISTANCE)).intValue();
        MAX_MARK_DISTANCE = ((Integer)doc.getProperty(
                                 SettingsNames.MAX_MARK_DISTANCE)).intValue();
        MIN_MARK_DISTANCE = ((Integer)doc.getProperty(
                                 SettingsNames.MIN_MARK_DISTANCE)).intValue();
        SYNTAX_UPDATE_BATCH_SIZE = ((Integer)doc.getProperty(
                                        SettingsNames.SYNTAX_UPDATE_BATCH_SIZE)).intValue();
    ...
    */
    synchronized void insertMark(Mark mark, int offset)
    throws BadLocationException, InvalidMarkException {
        if (offset < 0 || offset > docLen) {
            throw new BadLocationException(WRONG_POSITION + offset
                + DOC_LEN + docLen, offset);
        }
    ...
    * document is read.
    */
    void insertMark(Mark mark, int offset, int line)
    throws BadLocationException, InvalidMarkException {
        mark.offset = offset;
        mark.line = line;
        marks.insert(mark);
    ...
    */
    ...
    /** Get the first mark of the specified class that lies
    * on the given position.
    * @param offset position at which the mark is searched
    * @param markClass the class that the mark must be of. It can
    *  be also an instance of a descendant of this class
    *  (<tt>Class.isInstance</tt> is used).
    ...
    */
    ...
    void initialReadUpdate() { // doesn't need to be synced
        ...
        updatedAreaEnd = 0; // signal that all the syntax marks need to be updated
    }
    ...
    void checkEOLMark(int pos) {
        try {
            ...
            if (eolMark.getOffset() != eolPos) { // mark at the wrong place
                ...
                try {
                    prepareSyntax(slot, syntax, mark, eolPos, 0, false); // scan up to eolMark ignoring it in search
                    // Fix of #11130 - need to move the mark AFTER the syntax is prepared
    ...
    */
    void prepareSyntax(final SyntaxSeg.Slot slot, final Syntax syntax,
    MarkFactory.SyntaxMark leftSyntaxMark, final int reqPos, int reqLen,
    boolean forceLastBuffer, boolean forceNotLastBuffer) throws BadLocationException {

        /* Check whether the syntax marks in the area are already updated
         * The end of the requested area is checked although it would
         * be enough to scan till markPos. However it's better for drawing
         * because it doesn't know the end of the last token to draw
         * and therefore it searches through syntax-marks at the end
         * of the drawn area to find the syntax-mark that lays
         * (including pre-scan) after the area end.
         */
        if (updatedAreaEnd >= 0 && updatedAreaEnd < reqPos + reqLen) {
            ...
            // Compute req-len for marks updating. It is batched to update more marks at once
            final int umReqLen = Math.min(reqLen + SYNTAX_UPDATE_BATCH_SIZE, docLen - reqPos);
            slot.load(doc, slotLoadPos, updLen + umReqLen);
            syntax.load(updStateInfo, slot.array, slot.offset + updPreScan,
                        updLen - updPreScan, (reqPos == docLen), updMarkPos);
            ...
            marks.render(
                new DocMarks.Renderer() {
                    public void render() {
                        ...
                        int index = (updSyntaxMark != null)
                            ? (getNextIndex(getMarkIndex(updSyntaxMark)))
                            : getNextIndex(-1);
                        int pos = umPos;
                        Mark markArray[] = getMarkArray();
                        ...
                        while (index < markCnt) { // possibly till end of mark array
                            ...
                            if (pos > reqPos + umReqLen) { // reached the end of area available in slot
                                index = prevIndex; // this could be a syntax mark, so go back for further test
                                break;
                            }
                            ...
                            if (mark instanceof MarkFactory.SyntaxMark) {
                                ...
                                while (syntax.nextToken() != null) { } // scan till this mark

                                syntaxMark.updateStateInfo(syntax); // update state-info of the mark
                                updatedAreaEnd = pos; // this area is up-to-date
                                lastPos = pos; // move to next area between syntax marks
                            }
                        }
        ...
        syntax.load(stateInfo, slot.array, slot.offset + preScan,
                    prepareLen - preScan, forceNotLastBuffer ? false : (reqPos >= docLen), reqPos);

        // go through all the tokens till the required position
        while (syntax.nextToken() != null) { }

        syntax.relocate(slot.array, slot.offset + prepareLen, reqLen,
                        forceNotLastBuffer ? false : forceLastBuffer || (reqPos + reqLen >= docLen), reqPos + reqLen);
    }
