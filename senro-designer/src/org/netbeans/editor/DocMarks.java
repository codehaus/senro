 ...
 * in the array with a gap similar like in {@link javax.swing.text.GapContent}.
 *
...
final class DocMarks {
    ...
    /** Number of line separators corresponding
     * to the fictional document's data gap.
     */
    private int dataGapLineCount;
    
    /** Number of marks that are still in the marks array but 
     * that have the valid flag set to false.
     */
    ...
    synchronized void insert(Mark mark) {
        ...
        if (offset > dataGapStart
            || (offset == dataGapStart && !backwardBias)
        ) {
    ...
    synchronized void dispose(Mark mark) {
        ...
        if (unusedMarksCount > Math.max(5, marks.length - (gapEnd - gapStart))) { // GC rule
            removeDisposedMarks();
        }
    }
     ...
     * @param reqSize the additional size requested. Typically equals to 1.
     * @param ind index to which the gap is moved.
     * @return relocated index in the new marks array that corresponds
     ...
     * data gap must be updated by the modifiaction. The data gap can
     * possibly be moved and the marks in the moved area must
     * be updated accordingly.
     * @param offset offset of the modification
     * @param line line of modification. It is populated for removals only.
     ...
     * @param lineCount number of line separators in the added/removed part.
     *  It is &lt;0 for removals.
     */
    synchronized void update(int offset, int line, int length, int lineCount) {
        ...
        if (dataGapStart < offset) { // current data gap below offset
            ...
            while (true) {
                while (ind <= bound) {
                    Mark mark = marks[ind];
                    if (mark.offset > offsetAfterGap
                        || (mark.offset == offsetAfterGap && !mark.backwardBias)
                    ) {
            ...
            while (true) { // possibly two passes (above and under gap)
                while (ind > bound) {
                    ...
                    if (mark.offset < offset
                        || (mark.offset == offset && mark.backwardBias)
                    ) {
            ...
            ind = (length < 0)
                ? findInsertIndex(offset, true)
                : 0; // index no important for inserts
        }
        ...
        if (length < 0) { // remove performed
             ...
             * will be moved to the offset (before data gap).
             ...
             * The marks with the forward bias are placed
             * after data gap.
             */
            int lastBBMInd = -1; // index of last backward bias mark
            int bound = (ind >= gapEnd) ? gapEnd : 0; // boundary for the first pass
            offset += length; // offset back to original value
            int offsetAfterGap = offset
                + (dataGapEnd - dataGapStart); // use current (updated) gap size
    ...
    synchronized int getOffset(Mark mark) {
        ...
        return (offset <= dataGapStart)
            ? offset
            : (offset - (dataGapEnd - dataGapStart));
    }
    
    synchronized int getLine(Mark mark) {
        // offset == dataGapStart for backward bias marks
        // forward bias marks == dataGapEnd instead
        return (mark.offset <= dataGapStart)
            ? mark.line
            : (mark.line - dataGapLineCount);
    }

    /** Find the index of the given mark.
     * @param mark for which the index is being searched.
     ...
     * @param backwardBias whether the mark has backward or forward bias.
     * @return index &gt;= 0 and &lt;=<CODE>marks.length</CODE>
     *  in the marks array where the insert of the mark with the given
     *  offset and bias can be done.<BR> If there is more marks with the same
     *  offset and bias as the requested ones then the index after these marks
     *  is returned.
     *  The gapStart is preferred by the method over the gapEnd automatically.
     */
    int findInsertIndex(int offset, boolean backwardBias) {
        ...
        boolean belowGap = (gapStart > 0
            && getOffset(marks[gapStart - 1]) >= offset);
        ...
        while (low <= high) {
            ...
            if (markOffset < offset) {
                ...
                if (!backwardBias) { // forward bias required
                    /* Searched for offset increased by one
                     * so go to begining of all marks with
                     * (offset + 1)
                     */
                    ...
                    if (!mark.backwardBias) { // mark from bin-search has fwd bias
                        /* Find (backward) the first mark with lower offset
                         * or same offset but backward bias and go one
                         * index back.
                         */
                        int bound = belowGap ? 0 : gapEnd;
                        while (true) {
                        ...
                        /* Goto the end of the marks with the same offset
                         * and backward bias by finding the first mark
                         * with the same offset but forward bias
                         * or mark with greater offset.
                         */
     ...
     * @param markClass class of the mark to be found. It can be null
     *  to accept any mark.
     */
...
//        System.err.println("getLeftMark() offset=" + offset + ", markClass=" + markClass + ", found mark=" + ret);
//        if (ret != null) {
//            System.err.println("    markOffset=" + getOffset(ret) + ", line=" + getLine(ret));
//        }
     ...
     * @param offset offset where the mark should be found.
     * @param markClass class of the mark to be found. It can be null
     *  to accept any mark.
     */
    ...
    */
    synchronized Mark getMarkFromLine(int line) {
        boolean aboveGap = (gapEnd <= marksLengthM1
            && getLine(marks[gapEnd]) <= line);
    ...
    */
    static abstract class Renderer {
        ...
        Mark[] copyAllMarks() {
            ...
            System.arraycopy(marks.marks, marks.gapEnd, ret, marks.gapStart,
                marks.marks.length - marks.gapEnd);
            return ret;
        }
    ...
    /** Check whether the marks offsets and lines are sorted
     * correctly.
     */
    void check() {
        int ind = 0;
        ...
        while (true) {
            while (ind <= bound) {
                ...
                if (mark.offset < lastOffset
                    || (mark.offset == lastOffset 
                        && (mark.backwardBias && !lastBackwardBias)
                    )
                ) {
    ...
    private void consistencyError(boolean offsetError, int ind) {
        throw new IllegalStateException(
            "DocMarks.check(): "
            + (offsetError ? "Offset" : "Line")
            + " inconsistency found at ind=" + ind
            + ", mark=" + marks[ind]
        );
    }
    ...
    public String toString() {
        return "marksCount=" + getMarksCount()
            + ", gapStart=" + gapStart
            + ", gapEnd=" + gapEnd
            + ", dataLen=" + (Integer.MAX_VALUE - (dataGapEnd - dataGapStart))
            + ", dataGapStart=" + dataGapStart
            + ", dataGapEnd=" + dataGapEnd
            + ", dataGapLineCount=" + dataGapLineCount;
    }
