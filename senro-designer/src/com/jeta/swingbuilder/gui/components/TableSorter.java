...
public class TableSorter extends TableMap 
{
   ...
   public void checkModel() 
   {
	 ...
	 {
	    System.err.println("Sorter not informed of a change in model:  indexes.length = " + m_indexes.length + "  model.rowCount = " + model.getRowCount() );
	 }
      }
   }
   ...
   /**
    * Handles data inserted into the model.  We update all indexes affected
    * by the insert (but we don't resort).  
    * @param firstRow the first row of the range of rows that were inserted
    * @param lastRow the last row of the range of rows that were inserted
    */
   void handleDataDeleted( int firstRow, int lastRow )
   {
      ...
      // we need to shift all affected indexes up as well as correct the index values
   ...
   void handleDataInserted( int firstRow, int lastRow )
   {
	 ...
	 // we need to shift all affected indexes down as well as correct the index values
   ...
   public void setModel(TableModel model) 
   {
      ...
      model.addTableModelListener( new TableModelListener()
	 {
	    public void tableChanged(TableModelEvent e) 
	    {
	       ...
	       {
		  // data was added to model, so let's update only those indexes
		  // that were affected
		  ...
		  handleDataInserted( firstrow, lastrow );
	       ...
	       {
		  // data was removed from the model, so let's update only those indexes
		  // that were affected
		  int firstrow = e.getFirstRow();
		  int lastrow = e.getLastRow();
		  handleDataDeleted( firstrow, lastrow );
	       ...
	       {
		  //		  System.out.println( "TableSorter got unknown event:  sortmode = " + m_sortmode + "  col = " + m_sortingColumn );
		  // assume that the entire table has changed
		  reallocateIndexes();
		  if ( m_sortmode != SortMode.NONE )
		     sort(this);
	       }
	       // we don't worry about updates, the user must manually resort if
