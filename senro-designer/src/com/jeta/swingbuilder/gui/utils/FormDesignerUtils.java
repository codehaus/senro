...
public class FormDesignerUtils {
	...
	/**
	 * Checks if the span can be set to the given values for the given component.
	 */
	...
	/**
	 * Converts dialog units in the x and y to pixels. This is used for providing
	 * a consistent initial window size for growable panels.
	 */
	...
	public static String fastTrim(String str) {
		...
		if (str.charAt(0) == ' ' || str.charAt(len - 1) == ' ')
			...
			return str; // no leading/trailing spaces so just return the original
							// string
	}
	...
	public static void setSpan(GridComponent gc, int colspan, int rowspan) {
		...
		if (checkSpan(gc, colspan, rowspan)) {
			ReadOnlyConstraints rocc = new ReadOnlyConstraints(colstart, rowstart, colspan, rowspan, cc
					.getHorizontalAlignment(), cc.getVerticalAlignment(), cc.getInsets());
	...
	/**
	 * Saves the specified form to the specified file.  If the file name ends in .xml, the form
	 * is stored in xml format.  Otherwise, it is stored in binary form.
	 * @param fpackage
	 * @param file
	 * @throws IOException
	 * @throws JMLException 
