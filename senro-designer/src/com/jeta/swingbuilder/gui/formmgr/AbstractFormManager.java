...
public abstract class AbstractFormManager implements FormManager {
	...
	/**
	 * Activates an AWT container. We search for all child components and replace
	 * any nested forms with a surrogate. This is needed for components like
	 * JTabbedPanes where we can embed a form at design time.
	 */
	 ...
	 * any nested forms are synchronized with the latest changes in the new view.
	...
	public void deactivateForm(FormComponent form) {
		...
		while (iter.hasNext()) {
			...
			if (gc instanceof FormComponent) {
				...
				/**
				 * We register the form each time because the child form could be
				 * embedded an not registered with the form manager yet.
				 */
	...
	/**
	 * DesActivates the forms in the given editor. Additionally, any nested forms
	 * are synchronized with the latest changes in the new view. This is
	...
	/**
	 * Searches the list of cached forms for a form that has the given file path.
	 */
	 ...
	 * need the componentsouce because some handlers are responsible for creating
	 * child components in the form.
	 */
	public void installFormHandlers(ComponentSource compsrc, FormComponent fc) {
		 ...
		 * custom bean is being used that itself uses a FormPanel. This is a very
		 * special case
		 */
		...
		try {
			...
			if (fc == null) {
				...
				if (fc == null) {
					...
					// @JMT no need to reset id's because the embedded form id is
				...
				// System.out.println( "AbstractFormManager.openForm found in cache:
				// " + file.getPath() );
			}
			return fc;
		} catch (Exception e) {
