...
import com.jeta.swingbuilder.gui.main.AboutView;
import com.jeta.swingbuilder.gui.main.FormEditorNames;
import com.jeta.swingbuilder.gui.main.FormPropertiesNames;
import com.jeta.swingbuilder.gui.main.MainFrameNames;
...
public class IBMainFrameController extends IBFormEditorController
{
     ...
     * @param frame
     */
    IBMainFrameController(IBMainFrame frame)
    {
    ...
    private void copyToClasspath(Collection files)
    {
        try {
            ...
            if (pmodel != null) {
                ...
                String projfilename = pmodel.getProjectPath();
                File projf = new File(projfilename);
                String project_parent = projf.getParent();
                String check_path = project_parent + File.separatorChar + classpath;
                File f = new File(check_path);
                if (f.isDirectory()) {
                    classpath = check_path;
                } else {
     ...
     *
     * @return CLOSE_OK(0) if the form was successfully closed CLOSE_CANCELED (1)
    ...
    public File saveForm(boolean saveAs)
    {
        ...
        /**
         * the list of form files that we are saving. This is the current form and
         * any linked forms
         */
        ...
        try {
            if (fc != null) {
                ...
                /** now, save linked forms that are located in the form we are saving */
    ...
    {
        DefaultLookAndFeelManager dfm = (DefaultLookAndFeelManager) JETARegistry
     ...
     */
    public static void setLookAndFeel(IBMainFrame mainframe, JFrame runframe, LookAndFeelInfo lookandfeel)
    {
        try {
            if (lookandfeel == null) {
                return;
    ...
    public class ShowUndoManagerAction implements ActionListener
    {
        ...
        {
            if (m_frame == null) {
                m_frame = new com.jeta.swingbuilder.gui.undo.UndoManagerFrame(IBMainFrameController.this.m_frame);
                m_frame.setSize(600, 400);
                m_frame.setLocation(150, 150);
            }
            m_frame.reload();
