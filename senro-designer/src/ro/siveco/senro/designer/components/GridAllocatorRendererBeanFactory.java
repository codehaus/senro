...
public class GridAllocatorRendererBeanFactory extends JComponentBeanFactory
{
    private static final Set<String> BASIC_PROPERTIES =
        Collections.unmodifiableSet(new HashSet<String>(Arrays.asList("gridAllocator", "id", "rowExpr", "columnExpr")));

    public GridAllocatorRendererBeanFactory()
    {
        super(GridAllocatorRenderer.class);
    }
    ...
    public JETABean createBean(String compName, boolean instantiateBean, boolean setDefaults) throws FormException
    {
        Component comp = null;
        if (instantiateBean) {
            comp = new GridAllocatorRenderer();
            GridAllocatorRenderer gar = (GridAllocatorRenderer) comp;
            gar.setName(compName);
        }
        DynamicBeanInfo beaninfo = JComponentBeanFactory.createBeanInfo(GridAllocatorRenderer.class);
